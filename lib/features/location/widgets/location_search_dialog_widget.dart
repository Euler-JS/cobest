...
import 'package:cobes_marketplace/features/location/domain/models/prediction_model.dart' as prediction_model;
      ...
      child: Material(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
        child: SizedBox(width: 1170, child: TypeAheadField(
          ...
          itemBuilder: (context, prediction_model.Suggestions suggestion) {
            return Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
              child: Row(children: [
                ...
                Expanded(child: Text(suggestion.placePrediction?.text?.text ?? '' , maxLines: 1, overflow: TextOverflow.ellipsis,
                  style: Theme.of(context).textTheme.displayMedium!.copyWith(
                  color: Theme.of(context).textTheme.bodyLarge!.color, fontSize: Dimensions.fontSizeLarge))),
              ]),
            );
          ...
          onSelected: (prediction_model.Suggestions suggestion) {
            Provider.of<LocationController>(context, listen: false).setLocation(suggestion.placePrediction?.placeId,
